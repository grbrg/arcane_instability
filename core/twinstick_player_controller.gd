...
func handle_joypad_button(event: InputEventJoypadButton) -> void:
	...
	if event.is_pressed():
		match event.button_index:
			JOY_BUTTON_LEFT_SHOULDER: player.request_spell(0)
			JOY_BUTTON_RIGHT_SHOULDER: player.request_spell(1)
	else:
		match event.button_index:
			JOY_BUTTON_LEFT_SHOULDER: player.release_spell(0)
			JOY_BUTTON_RIGHT_SHOULDER: player.release_spell(1)
